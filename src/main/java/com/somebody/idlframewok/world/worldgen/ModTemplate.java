...
//    /**
//     * Add blocks and entities from this structurebig to the given world, restricting placement to within the chunk
//     * bounding box.
//     *
...
//    }
    ...
    public void addBlocksToWorld(World worldIn, BlockPos pos, @Nullable ITemplateProcessor templateProcessor, PlacementSettings placementIn, int flags){
        addBlocksToWorld(worldIn, pos, templateProcessor, placementIn, flags, true);//edited
    }

    public void addBlocksToWorld(World worldIn, BlockPos pos, @Nullable ITemplateProcessor templateProcessor, PlacementSettings placementIn, int flags, boolean cascadeProtect)
    {
        int tempFlag = flags;
            ...
            for (Template.BlockInfo template$blockinfo : this.blocks)
            {
                ...
                if (template$blockinfo1 != null)
                {
                    ...
                    if ((block == null || block != block1) && (!placementIn.getIgnoreStructureBlock() || block1 != Blocks.STRUCTURE_BLOCK) && (structureboundingbox == null || structureboundingbox.isVecInside(blockpos)))
                    {
                        ...
                        if (cascadeProtect)
                        {
                            int x = blockpos.getX() % CHUNK_SIZE;
                            int z = blockpos.getZ() % CHUNK_SIZE;
                            if (x == 0 || z == 0 || z == 15 || x == 15)
                            {
                                tempFlag = WorldGenUtil.PREVENT_CASCADE_FLAGS_2;
                            }else {
                                tempFlag = flags;
                            }
                        }
                        ...
                        if (template$blockinfo1.tileentityData != null)
                        {
                            ...
                            if (tileentity != null)
                            {
                                ...
                                worldIn.setBlockState(blockpos, Blocks.BARRIER.getDefaultState(), tempFlag);
                            }
                        }

                        if (worldIn.setBlockState(blockpos, iblockstate1, tempFlag) && template$blockinfo1.tileentityData != null)
                        {
    ...
    {
        ModTemplate.BasicPalette template$basicpalette = new ModTemplate.BasicPalette();
        NBTTagList nbttaglist = new NBTTagList();
    ...
    public static class BlockInfo
    {
        ...
        /** The type of block in this particular spot in the structurebig. */
    ...
    public static class EntityInfo
    {
        ...
        /** the serialized NBT data of the entity in the structurebig */
