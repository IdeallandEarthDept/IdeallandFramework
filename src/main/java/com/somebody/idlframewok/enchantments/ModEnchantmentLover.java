...
import static com.somebody.idlframewok.util.EntityUtil.GetRandomAround;
...
public class ModEnchantmentLover extends ModEnchantmentBase {
    ...
    public ModEnchantmentLover(String name, Rarity rarityIn, EnumEnchantmentType typeIn, EntityEquipmentSlot[] slots) {
        ...
        setHidden(!ModConfig.ENCHANT_CONF.ENABLE_LOVER);
    }
    ...
    {
        if (event.isCanceled() || event.getEntityLiving().getEntityWorld().isRemote || !ModConfig.ENCHANT_CONF.ENABLE_LOVER_DEATH)
        {
            return;
        }
        ...
        if (lover != null)
        {
            CommonFunctions.broadcastbykey("idlframewok.msg.until_death", diedOne.getName(), lover.getName());
