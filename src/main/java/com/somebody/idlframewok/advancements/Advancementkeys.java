package com.somebody.idlframewok.advancements;

public class Advancementkeys {
//    public static String AD_HELLOSAKURA = "moremomostories/hellosakura";
//    public static String AD_ITZMXBLOCK="moremomostories/getitzmxblock";
//
//    public static String AD_CARD_DOUBLEHIT="moremomostories/doublehit";
//    public static String AD_CARD_INTERFERENCE="moremomostories/card_interference";
//    public static String AD_CARD_MYSTERIOUS="moremomostories/card_mysterious";
//    public static String AD_CARD_NEATURE="moremomostories/card_neature";
//
//    public static String AD_ITZMX2="moremomostories/itzmx2";
//    public static String AD_WINTER="moremomostories/winter";
//    public static String AD_SAKURADAY="moremomostories/sakuraday";
//
//    public static String AD_ZFPHIGH="moremomostories/zfphigh";
//    public static String AD_FIRETIME="moremomostories/goodfiretime";
//
//    public static String AD_PAPER1="moremomostories/paper1";
//    public static String AD_SCENE1="moremomostories/scene1";
//    public static String AD_DREAM_1="moremomostories/dream1";
//    public static String AD_AGAIN_1="moremomostories/again1";
//    public static String AD_LX="moremomostories/lx";
//
//    public static String AD_MEETVAN="moremomostories/meetvan";
//
//    public static String AD_OMOD_ROOT="other_mod/meetvan";

}
