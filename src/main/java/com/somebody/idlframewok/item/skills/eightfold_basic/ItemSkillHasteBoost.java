...
public class ItemSkillHasteBoost extends ItemSkillBase {
    public ItemSkillHasteBoost(String name) {
        super(name);
        cool_down = 50f;
        maxLevel = 4;
    ...
    public ActionResult<ItemStack> onItemRightClick(World worldIn, EntityPlayer playerIn, EnumHand handIn) {
        ...
        if (isStackReady(playerIn, stack))
        {
            if (!worldIn.isRemote)
            {
                playerIn.addPotionEffect(new PotionEffect(MobEffects.HASTE, (int)(getDura(stack) * TICK_PER_SECOND), IDLSkillNBT.getLevel(stack) - 1));
